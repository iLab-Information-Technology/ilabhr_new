<ul>
    ...
    @endphp
    @if (
        (in_array('drivers', user_modules()) ||
            in_array('businesses', user_modules()) ||
            in_array('coordinatorReports', user_modules())) &&
            ($sidebarUserPermissions['view_drivers'] != 5 ||
                $sidebarUserPermissions['view_businesses'] != 5 ||
                $sidebarUserPermissions['view_coordinator_reports'] != 5) &&
            ($sidebarUserPermissions['view_drivers'] != 'none' ||
                $sidebarUserPermissions['view_businesses'] != 'none' ||
                $sidebarUserPermissions['view_coordinator_reports'] != 'none'))
        <x-menu-item icon="people" :text="__('app.menu.dms')">
            ...
            <div class="accordionItemContent pb-2">
                ...
                @if (in_array('branches', user_modules()) &&
                        $sidebarUserPermissions['view_branches'] != 5 &&
                        $sidebarUserPermissions['view_branches'] != 'none')
                    <x-sub-menu-item :link="route('branches.index')" :text="__('app.menu.branches')" />
                @endif

                @if (in_array('driverTypes', user_modules()) &&
                        $sidebarUserPermissions['view_driver_types'] != 5 &&
                        $sidebarUserPermissions['view_driver_types'] != 'none')
                    <x-sub-menu-item :link="route('driver-types.index')" :text="__('app.menu.driver_types')" />
                @endif
                @if (in_array('drivers', user_modules()) &&
                        $sidebarUserPermissions['view_drivers'] != 5 &&
                        $sidebarUserPermissions['view_drivers'] != 'none')
                    <x-sub-menu-item :link="route('drivers.index')" :text="__('app.menu.drivers')" />
                @endif
                @if (in_array('businesses', user_modules()) &&
                        $sidebarUserPermissions['view_businesses'] != 5 &&
                        $sidebarUserPermissions['view_businesses'] != 'none')
                    <x-sub-menu-item :link="route('businesses.index')" :text="__('app.menu.businesses')" />
                @endif
                @if (in_array('coordinatorReports', user_modules()) &&
                        $sidebarUserPermissions['view_coordinator_reports'] != 5 &&
                        $sidebarUserPermissions['view_coordinator_reports'] != 'none')
                ...
                @endphp
                @if (in_array('revenueReporting', user_modules()) &&
                        $sidebarUserPermissions['view_revenue_reporting'] != 5 &&
                        $sidebarUserPermissions['view_revenue_reporting'] != 'none')
                    <x-sub-menu-item :link="route('dms.revenue-reporting.index')" :text="__('app.menu.revenue_reporting')" />
                @endif
                @if (in_array('receiptVoucher', user_modules()) &&
                        $sidebarUserPermissions['view_receipt_voucher'] != 5 &&
                        $sidebarUserPermissions['view_receipt_voucher'] != 'none')
    ...
    <x-menu-item icon="people" :text="__('app.menu.vms')">
        <x-slot name="iconPath">
            <path
                d="M15 14s1 0 1-1-1-4-5-4-5 3-5 4 1 1 1 1h8zm-7.978-1A.261.261 0 0 1 7 12.996c.001-.264.167-1.03.76-1.72C8.312 10.629 9.282 10 11 10c1.717 0 2.687.63 3.24 1.276.593.69.758 1.457.76 1.72l-.008.002a.274.274 0 0 1-.014.002H7.022zM11 7a2 2 0 1 0 0-4 2 2 0 0 0 0 4zm3-2a3 3 0 1 1-6 0 3 3 0 0 1 6 0zM6.936 9.28a5.88 5.88 0 0 0-1.23-.247A7.35 7.35 0 0 0 5 9c-4 0-5 3-5 4 0 .667.333 1 1 1h4.216A2.238 2.238 0 0 1 5 13c0-1.01.377-2.042 1.09-2.904.243-.294.526-.569.846-.816zM4.92 10A5.493 5.493 0 0 0 4 13H1c0-.26.164-1.03.76-1.724.545-.636 1.492-1.256 3.16-1.275zM1.5 5.5a3 3 0 1 1 6 0 3 3 0 0 1-6 0zm3-2a2 2 0 1 0 0 4 2 2 0 0 0 0-4z" />
        </x-slot>
        <div class="accordionItemContent pb-2">
            <x-sub-menu-item :link="route('vehicle-types.index')" :text="__('app.menu.vehicle_type')" />
    ...
    @if (in_array('admin', user_roles()))
        <x-menu-item icon="clipboard" :text="__('app.menu.activity')" :link="route('activity-log.index')">
            <x-slot name="iconPath">
                <path
                    d="M4 1.5H3a2 2 0 0 0-2 2V14a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V3.5a2 2 0 0 0-2-2h-1v1h1a1 1 0 0 1 1 1V14a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V3.5a1 1 0 0 1 1-1h1v-1z" />
                <path
